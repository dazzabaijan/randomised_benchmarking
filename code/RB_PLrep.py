...
def operator_groups2(n):
    ...
    if n == "pauli":
        ...
        non_cliff = non_clifford_group(n)
        non_cliff = np.array([np.kron(non_cliff[i], non_cliff[i].conj())
                             for i in range(len(non_cliff))])
        return non_cliff
        # return gate_set[np.random.choice(gate_set.shape[0], sample_size)]
...
def create_rotation(angles: np.ndarray) -> np.ndarray:
    ...
    rotation = np.identity(4, dtype=complex)
...
def gen_sigma2(minimum, maximum, ave, sigma, max_size):
    ...
    # print(len(lambda1), len(lambda2), len(lambda1))
...
def randomized_benchmarking2(input_state: np.ndarray, seq_len: int,
    ...
    gate_set = operator_groups2(n)
    rand_arr1 = np.random.choice(gate_set.shape[0], (seq_len, samp_size))

    for j in range(1, seq_len+1):
        q_gates = gate_set[rand_arr1[j-1,:]]
...
def var_graph(exps, end_point, stepsize, input_state, noise_mean,
...
#    import pdb; pdb.set_trace()
    # remember to change this if u change get_data(200) above
...
#                           np.array([[0],[0],[0],[1]]))
# length, fidelity, seq_len = get_data2(np.array([[1],[0],[0],[0]]), 80,
#                                       10, 1, 0.1, 1, 3,
#                                       np.array([[0],[0],[0],[1]]))
