...
def randomized_benchmarking2(input_state: np.ndarray, seq_len: int,
    ...
    # inverse_gate2 = np.array(noisy_seq[::-1])[0]
    # for el in np.array(noisy_seq[::-1])[1:]:    
    #     inverse_gate2 = inverse_gate2@el
    
    # inverse_gate3 = np.array(noisy_seq2[::-1])[0]
    # for el in np.array(noisy_seq2[::-1])[1:]:    
    #     inverse_gate3 = inverse_gate3@el
    
    seq_adjoint = inverse_gate.transpose(0, 2, 1).conj()
    
    # print("inv_gate: {}".format(inverse_gate))
    # print("seq_adjoint: {}".format(seq_adjoint))
    # print("rho_tensor: {}".format(rho_tensor))
    # output_state_up = seq_adjoint@inverse_gate2@rho_tensor
    # output_state_down = seq_adjoint@inverse_gate3@rho_tensor_2
    
    output_state_up = seq_adjoint@channel1@rho_tensor
    output_state_down = seq_adjoint@channel2@rho_tensor_2    
    
    # print(output_state_up)
    # print(output_state_down)
    # measure = init_tensor(np.array([X.flatten()]), samp_size)
    measure_up = init_tensor(np.array([Z.flatten()]),samp_size)
    measure_down = init_tensor(np.array([Z.flatten()]),samp_size)
    
    expected_val_up = measure_up@output_state_up
    expected_val_down = measure_down@output_state_down
    # print(expected_val_up)
    # print(expected_val_down)
    ave_fid = (np.average(expected_val_up)-np.average(expected_val_down))/2
    # print(np.real(ave_fid))
